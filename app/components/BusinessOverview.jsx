  ...
  return (
    <section className="relative w-full py-32 px-6 md:px-12 bg-paper overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        {loading ? (
          <p className="text-steel text-center py-16">Loading...</p>
        ) : cards.length === 0 ? (
          <p className="text-steel text-center py-16">Content coming soon.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {cards.map((item, index) => {
              const Icon =
                OVERVIEW_ICONS[item.icon] ?? OVERVIEW_ICONS.ShieldCheck;
              return (
                <motion.div
                  key={item.id}
